var input = [[8,2,22,97,38,15,0,4,0,75,4,5,7,78,52,12,5,77,91,8],
...
function checkLeft(column,row){
	...
	var tempTotal = input[row][column]*input[row][column-1]*input[row][column-2]*input[row][column-3];
	if(maxProduct<tempTotal){
...
function checkUp(column,row){
	if(row<3){
		return;
	}
	var tempTotal = input[row][column]*input[row-1][column]*input[row-2][column]*input[row-3][column];
	if(maxProduct<tempTotal){
		maxProduct = tempTotal;
		maxRow = row;
		maxColumn = column;	
...
function checkUpLeft(column,row){
	if(row<3 || column<3){
		return;
	}
	var tempTotal = input[row][column]*input[row-1][column-1]*input[row-2][column-2]*input[row-3][column-3];
	if(maxProduct<tempTotal){
		maxProduct = tempTotal;
		maxRow = row;
		maxColumn = column;	
...
function checkUpRight(column,row){
	if(row<3 || column>16){
		return;
	}
	var tempTotal = input[row][column]*input[row-1][column+1]*input[row-2][column+2]*input[row-3][column+3];
